 ...
 *   const F = new FlightHud({ id: 'engFlight', scene, THREE });  F.show();  F.update(ball);  F.arc(points)
 */
export class FlightHud {
  constructor({ id = 'engFlight', scene = null, THREE = null } = {}) {
    this.id = id; this.scene = scene; this.THREE = THREE; this.on = false; this.el = null; this.line = null; this._last = '';
  ...
  _mount() {
    if (!document.getElementById(this.id + 'Css')) {
      ...
      st.textContent = `
#${this.id}{position:fixed;left:14px;bottom:14px;z-index:26;font:11px/1.45 ui-monospace,Menlo,Consolas,monospace;color:#d9f3ff;background:rgba(6,10,18,.74);border:1px solid rgba(159,240,255,.25);border-radius:10px;padding:8px 10px;min-width:262px;pointer-events:none;backdrop-filter:blur(6px);display:none}
#${this.id} .fl-h{font-weight:800;letter-spacing:.14em;font-size:10px;color:#9ff0ff;margin-bottom:4px;display:flex;justify-content:space-between}
...
#${this.id} .good{color:#a6ff5d}#${this.id} .warn{color:#ffe08a}#${this.id} .bad{color:#ff8a8a}#${this.id} .cool{color:#9ff0ff}#${this.id} .hot{color:#ffb15d}`;
    ...
    const el = document.createElement('div'); el.id = this.id; el.innerHTML = '<div class="fl-h"><span>FLIGHT</span><span class="cool">physics</span></div><div class="fl-b"></div>';
  ...
  update(ball) {
    ...
    rows.push(['state', `<span class="${held ? 'warn' : S.asleep ? 'cool' : 'hot'}">${state}</span>`]);
    ...
    const Rl = L.release;
    if (Rl) rows.push(['release', `${Rl.source || ''} ${f(Rl.speed)} m/s · elev ${f(Rl.elev, 0)}°` + (Rl.handSpeed != null ? ` · hand ${f(Rl.handSpeed)} m/s (${Rl.why || ''})` : '')]);
    if (ball.intent) { const it = ball.intent; rows.push(['intent', `<span class="${it.state === 'push' ? 'hot' : it.state === 'claw' ? 'good' : 'cool'}">${it.state}</span> · conf ${f(it.conf, 2)}${it.blockAttract ? ' · <span class="warn">no-glue</span>' : ''}`]); }
  ...
  /** the predicted arc (points [[x,y,z], …]); [] hides it */
  arc(points) {
    ...
    if (!this.on || !points || points.length < 2) { this.line.visible = false; return; }
