...
export class WorldAgent {
  ...
  _system(guide) {
    ...
    return [
      ...
      "THE SKY: a skybox (set_skybox) is the world's enclosing sky shell. It is ALWAYS auto-centred on the world and auto-sized to fully enclose it — it can never be left small, off-centre, or inside the building; the engine re-fits it automatically whenever the world is resized. It is a recallable object named 'sky' (id __sky__) that appears in objects[] like any other. To resize the SKY, target the sky: scale_object {label:'sky', factor:2} (or set_transform {label:'sky', scale:…}). NEVER use transform_scene for the sky — transform_scene resizes the building/environment, which is a DIFFERENT object. If the user says the sky is too small, scale the sky up via scale_object on 'sky' (or just re-call set_skybox, it auto-encloses). The sky stays locked to the world centre; you cannot move it.",
